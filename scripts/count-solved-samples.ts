...
export const SOLVE_COUNT_HELP = `Usage: bun run solve-count [--chip am62l|rk3308|k230|imx6ull|t113s3|all] [--sample ID|EXIT]
...
selects rk3308, k230, imx6ull, or t113s3. Use a unique ID when combining --sample with --chip all.
...
export function selectSamples(args: readonly string[]): {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (argument === "--chip") {
      ...
      if (
        ...
        throw new Error(
          `Unknown chip ${value}; expected am62l, rk3308, k230, imx6ull, t113s3, or all`,
        )
      }
      chip = value
    } else {
  ...
  if (samples.length > 1) {
    throw new Error(
      `Exit ${sampleSelector} matches multiple chips; use a sample ID or select --chip am62l|rk3308|k230|imx6ull|t113s3`,
    )
  }
  if (samples.length === 0) {
