...
import { getRk3308MemoryPlacement } from "../lib/create-rk3308-fanout-sample"
import {
  DDR3L_PAD_DIAMETER,
  DDR3L_PAD_POSITIONS,
  DDR3L_SIGNAL_BALLS,
} from "../lib/ddr3l-footprint"
import { RK3308_DDR_SIGNAL_BALL_MAP } from "../lib/rk3308-ball-map"
...
export function generateRk3308PreviewSvg(): string {
  const width = 1032
  const margin = 24
  const gap = 12
  ...
  const originY = 154
  const rowCount = Math.ceil(RK3308_FANOUT_DIRECTION_CASES.length / 3)
  const height = startY + rowCount * (panelHeight + gap) - gap + margin

  const panels = RK3308_FANOUT_DIRECTION_CASES.map((directionCase, index) => {
    const placement = getRk3308MemoryPlacement(directionCase)
    const x = margin + (index % 3) * (panelWidth + gap)
    const y = startY + Math.floor(index / 3) * (panelHeight + gap)
    const label = `${directionCase.id.split("-")[0]}  ${directionCase.name.replace("RK3308 · ", "")}`
...
<text x="${originX}" y="${number(originY + 8.8 * scale)}" class="chip-label">RK3308</text>
...
<title id="title">Twelve RK3308 to DDR3L package placements</title>
<desc id="description">Each panel keeps the 355-ball Rockchip RK3308 fixed and moves the 96-ball Samsung DDR3L package to a different edge or offset. Orange pads mark the 49 DDR interface signals on each chip. Positions and RAM rotations come from the sample source. No routes are shown.</desc>
<style>
...
<path d="M-6.5 5.3V6.5H-5.3" class="pin-one"/>
<g class="pads">${drawPads(RK3308_PAD_POSITIONS, 0.15, (ballName) => ballName in RK3308_DDR_SIGNAL_BALL_MAP)}</g></g>
...
<text x="24" y="36" class="title">RK3308 → DDR3L</text>
<text x="24" y="60" class="subtitle">355-ball Rockchip SoC · 96-ball ×16 RAM · 12 physical placements</text>
...
if (import.meta.main) {
  ...
  const output = new URL("rk3308-placements.svg", directory)
  await Bun.write(output, generateRk3308PreviewSvg())
