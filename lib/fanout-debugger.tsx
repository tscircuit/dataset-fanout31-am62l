...
export type FanoutDebuggerSample = Omit<
  Am62lFanoutSample,
...
export function FanoutDebugger({
  createSample,
  chipName,
  ballCount,
  packageLabel,
  ...
  const planeDropCount = planeBuses.reduce(
    (total, bus) => total + bus.connectionNames.length,
    0,
  )
  const signalCount = sample.simpleRouteJson.connections.length - planeDropCount
  const signalBusCount = buses.length - planeBuses.length
  const differentialPairCount =
    sample.simpleRouteJson.differentialPairs?.length ?? 0
