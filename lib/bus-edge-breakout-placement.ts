 ...
 */
export function createBusEdgeBreakoutPlacement(
  exits: Readonly<Record<string, Exclude<FanoutExitPosition, "center">>>,
): ImplicitBreakoutPointSolverFn {
  return (input) => {
    if (input.regions.length !== 1)
      throw new Error("edge placement expects one source region")
    ...
    for (const edge of ["top", "right", "bottom", "left"] as const) {
      const buses = input.buses.filter((bus) => {
        ...
        if (!exit) throw new Error(`Missing edge for bus ${bus.busId}`)
      ...
      const connections = input.connections
        .filter((c) => {
          ...
          if (included.length && included.length !== members.length)
            throw new Error("Differential pair cannot span edges")
      ...
      for (const p of points) {
        if (assigned.has(p.connectionId))
          throw new Error(`Duplicate boundary connection ${p.connectionId}`)
    ...
    if (
      ...
      throw new Error("Incomplete boundary placement")
